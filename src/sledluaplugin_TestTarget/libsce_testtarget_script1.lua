...
et = {}
function DoCharlesStuff()
	x = 5
	local xx = x
	return x
end
setfenv(DoCharlesStuff, et)

function times(a, b, c)
	local result = timesWrapper(a, b, c)
	return result
end

function timesWrapper(a, b, c)
	local test = a
	local test2 = b
	local test3 = c	
	local sum = 0
	
	local charlesResult = DoCharlesStuff()
	...
	local actorBob = Actor.NewActor("bob")
	
	if actorBob:GetName() ~= "bill" and actorBob:GetAnimName() ~= "Sleeping" then
		anotherTemp = anotherTemp + 5
	end
	
	local actorBobTable = {}
	actorBobTable["test1"] = "String key"
	actorBobTable[actorBob] = "UserData key"
	actorBobTable[11] = "Number key"
	
	local blah = EnvironmentTest(a, b)
...
function EnvironmentTest(temp1, temp2)
	-- Get this functions environment
	...
	-- Create new global variable exclusive to this
	-- new environment
	var = {}
	var[1] = true
	var[2] = false
	...
	-- Create new function that will inherit
	-- new environment
	embed = function(temp1, temp2)
		local twenty = 20
		twenty = temp1 * temp2
	...
	local temp3 = embed(temp1, temp2)
	
	-- Reset functions environment
	setfenv(1, curEnv)
	
	local x = temp1 + temp2 + temp3
	x = x * temp1
	x = x * temp2
	x = x * temp3
	return x
