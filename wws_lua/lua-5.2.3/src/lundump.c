...
*/
void luaU_header (lu_byte* h, LuaDumpConfig *config)
{
 memcpy(h,LUA_SIGNATURE,sizeof(LUA_SIGNATURE)-sizeof(char));
 h+=sizeof(LUA_SIGNATURE)-sizeof(char);
 *h++=cast_byte(VERSION);
 *h++=cast_byte(FORMAT);
 *h++=cast_byte(config->endianness);
 *h++=cast_byte(config->sizeof_int);
 *h++=cast_byte(config->sizeof_size_t);
 *h++=cast_byte(sizeof(Instruction));
 *h++=cast_byte(config->sizeof_lua_Number);
