...
* make header
*/
void luaU_header (char* h, LuaDumpConfig *config)
{
 memcpy(h,LUA_SIGNATURE,sizeof(LUA_SIGNATURE)-1);
 h+=sizeof(LUA_SIGNATURE)-1;
 *h++=(char)LUAC_VERSION;
 *h++=(char)LUAC_FORMAT;
 *h++=(char)(config->endianness);
 *h++=(char)(config->sizeof_int);
 *h++=(char)(config->sizeof_size_t);
 *h++=(char)sizeof(Instruction);
 *h++=(char)(config->sizeof_lua_Number);
