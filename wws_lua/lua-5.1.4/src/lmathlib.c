...
static int math_deg (lua_State *L) {
  lua_pushnumber(L, (lua_Number)(luaL_checknumber(L, 1)/RADIANS_PER_DEGREE));
  return 1;
}
...
static int math_ldexp (lua_State *L) {
  lua_pushnumber(L, (lua_Number)ldexp(luaL_checknumber(L, 1), luaL_checkint(L, 2)));
  return 1;
}
...
static int math_min (lua_State *L) {
  ...
  lua_Number dmin = (lua_Number)luaL_checknumber(L, 1);
  int i;
  for (i=2; i<=n; i++) {
    lua_Number d = (lua_Number)luaL_checknumber(L, i);
    if (d < dmin)
      dmin = d;
  }
...
static int math_max (lua_State *L) {
  int n = lua_gettop(L);  /* number of arguments */
  lua_Number dmax = (lua_Number)luaL_checknumber(L, 1);
  int i;
  for (i=2; i<=n; i++) {
    lua_Number d = (lua_Number)luaL_checknumber(L, i);
    if (d > dmax)
...
static int math_random (lua_State *L) {
  ...
  switch (lua_gettop(L)) {  /* check number of arguments */
    ...
    case 1: {  /* only upper limit */
      ...
      lua_pushnumber(L, (lua_Number)(floor(r*u)+1));  /* int between 1 and `u' */
      break;
    }
    case 2: {  /* lower and upper limits */
      ...
      lua_pushnumber(L, (lua_Number)(floor(r*(u-l+1))+l));  /* int between `l' and `u' */
      break;
    }
